 ...
 *
 * Description:
 *   up_task_fork() is the architecture-specific base of task_fork().  It
 *   clones the calling task:  the child shares the parent's .data, .bss and
 *   heap -- it joins the parent's address environment, exactly as a pthread
 *   does -- but runs on a private copy of the parent's stack, and it runs
 *   concurrently with the parent.
 *
 *   This is neither fork() nor vfork().  See up_fork() and up_vfork().
 *
 * Returned Value:
 ...
 *
 * Description:
 *   up_vfork() is the architecture-specific base of vfork().  The child
 *   shares the parent's memory -- nothing is copied -- and the parent is
 *   suspended until the child calls _exit() or one of the exec family of
 *   functions.  The suspension is what makes the sharing safe, and it lives
 *   here in the kernel rather than in libc so that the parent is resumed at
 *   exec() as POSIX requires, and so that vfork() does not depend on
 *   CONFIG_SCHED_WAITPID.
 *
 *   Where CONFIG_ARCH_VFORK_STACK_BORROW is selected the child borrows the
 *   parent's stack outright rather than running on a relocated copy of it.
 *
 * Returned Value:
 ...
 *
 * Description:
 *   up_fork() is the architecture-specific base of POSIX fork().  The child
 *   receives its own copy of the parent's memory, at the same virtual
 *   addresses, by duplicating the parent's address environment.  The child
 *   may modify anything, may return from the function that called fork(),
 *   may call any function, and runs concurrently with the parent.
 *
 *   This requires an address environment that can be duplicated, so it is
 *   available only where CONFIG_ARCH_HAVE_FORK is selected.
 *
 * Returned Value:
 ...
 *
 * Description:
 *   Duplicate an address environment for POSIX fork().  Unlike
 *   up_addrenv_clone(), which copies only the representation and leaves both
 *   copies pointing at the same page tables and hence the same memory, this
 *   allocates fresh pages for the destination, copies the source's contents
 *   into them, and maps them at the same virtual addresses.  The result is
 *   an independent address environment holding an exact copy of the source's
 *   memory:  every pointer the source held into its own memory is still
 *   valid in the copy, and writes to one are invisible to the other.
 *
 *   Implemented only by architectures that select
 *   CONFIG_ARCH_HAVE_ADDRENV_FORK, which is what CONFIG_ARCH_HAVE_FORK is
 *   derived from.
 *
 * Input Parameters:
