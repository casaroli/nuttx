 ...
 *   child, and attach the duplicate to the child.  The child receives its
 *   own pages, holding a copy of the parent's contents, mapped at the same
 *   virtual addresses.
 *
 *   Contrast addrenv_join(), which gives the child the parent's memory, and
 *   up_addrenv_clone(), which merely copies the representation of an address
 *   environment and leaves both copies pointing at the same page tables.
 *
 * Input Parameters:
 *   ptcb - The tcb of the parent process.
 *   tcb  - The tcb of the child process.
