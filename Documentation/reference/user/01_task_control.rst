...
.. c:function:: pid_t vfork(void)
  ...
  in the parent's address space on borrowed time.
     ...
     On architectures that select ``CONFIG_ARCH_VFORK_STACK_BORROW`` the
     child borrows the parent's stack outright rather than running on a
     relocated copy of it.  That is required where stack frames contain
     absolute stack addresses -- the windowed ABI of Xtensa, whose register
     window save areas hold the spilled stack pointer of each frame -- and is
     a pure economy elsewhere.
