 ...
 *   is being performed is given by `type'; see include/nuttx/fork.h.  This
 *   architecture provides task_fork() and vfork() -- POSIX fork() needs an
 *   address environment to duplicate and there is none here.
 *
 *   Both of the ones it does provide give the child a stack of its own
 *   holding a relocated copy of the part of the parent's that is in use.
 *   The caller is always cloned as it stood at the call, because there is
 *   only one way to get here:  an ordinary function call into fork.S.  A
 *   flat build has no system-call boundary for the caller to be on the far
 *   side of, so there is no second path of the kind x86_64 needs.
 *
 *   The overall sequence is:
 *
 *   1) User code calls task_fork() or vfork().  The matching entry point in
 *      fork.S collects context information and transfers control to
 *      x86_fork().
...
pid_t x86_fork(const struct fork_s *context, int type)
{
