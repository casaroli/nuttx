...
int up_addrenv_fork(const arch_addrenv_t *parent, arch_addrenv_t *child)
{
  int ret;

  DEBUGASSERT(parent && child);

  memset(child, 0, sizeof(arch_addrenv_t));

  child->textvbase = parent->textvbase;
  child->datavbase = parent->datavbase;
  child->heapvbase = parent->heapvbase;
  ...
  ret = alloc_region(child->textpages, CONFIG_ARCH_TEXT_NPAGES,
                     (size_t)parent->ntext * MM_PGSIZE, &child->ntext);
  if (ret < 0)
    {
      goto errout;
    }

  ret = alloc_region(child->datapages, CONFIG_ARCH_DATA_NPAGES,
                     (size_t)parent->ndata * MM_PGSIZE, &child->ndata);
  if (ret < 0)
    {
      goto errout;
    }

  ret = alloc_region(child->heappages, CONFIG_ARCH_HEAP_NPAGES,
                     (size_t)parent->nheap * MM_PGSIZE, &child->nheap);
  if (ret < 0)
    {
      goto errout;
    }
  ...
  ret = copy_region(parent->textpages, child->textpages, child->ntext);
  if (ret < 0)
    {
      goto errout;
    }

  ret = copy_region(parent->datapages, child->datapages, child->ndata);
  if (ret < 0)
    {
      goto errout;
    }

  ret = copy_region(parent->heappages, child->heappages, child->nheap);
  if (ret < 0)
    {
      goto errout;
    }
