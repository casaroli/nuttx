 ...
 *   parent's stack.  It looks like the child is working -- it is even at
 *   the right
 *   offset -- until something returns through a slot the parent has since
 *   reused.
 *
 ...
 *
 * Description:
 *   Clone a caller that reached up_task_fork(), up_vfork() or up_fork() by
 *   an ordinary function call, so that the register snapshot fork.S took
 *   describes the caller itself.  That is the case in a flat build, and for
 *   a kernel thread in any build.
 *
...
static pid_t x86_64_fork_direct(struct tcb_s *parent,
                                const struct fork_s *context, int type)
{
  struct tcb_s *child;
  uint64_t newsp;
  uint64_t newfp;
 ...
 *
 * Description:
 *   Clone a caller that reached up_task_fork(), up_vfork() or up_fork()
 *   through a system call.  The register snapshot fork.S took is useless
 *   here:  it describes the kernel-side stub, so a child built from it would
 *   resume at a kernel address on a kernel stack.  What the caller was
 *   actually doing is the frame x86_64_syscall_entry() saved and
 *   x86_64_syscall() recorded in xcp.sregs; the child is built from that.
 *
...
static pid_t x86_64_fork_syscall(struct tcb_s *parent, int type)
{
   ...
   * address environment already holds a copy of the contents, or a borrowing
   * vfork() child.  There is then nothing to copy -- source and destination
   * would be the same region -- and nothing to relocate.
   */
 ...
 *
 * Description:
 *   The architecture-specific half of NuttX's three cloning primitives --
 *   task_fork(), vfork() and fork().  Which one is being performed is given
 *   by `type'; see include/nuttx/fork.h.  For fork() the semantics are those
 *   of POSIX fork(), except that the behavior is undefined if the process
 *   created either modifies any data other than a variable of type pid_t
 *   used to store the return value, or returns from the function in which
 *   fork() was called, or calls any other function before successfully
 *   calling _exit() or one of the exec family of functions.
 *
 *   The overall sequence is:
 *
 *   1) User code calls task_fork(), vfork() or fork().  The matching entry
 *      point in fork.S collects context information and transfers control to
 *      x86_64_fork().
...
pid_t x86_64_fork(const struct fork_s *context, int type)
{
   ...
   * nowhere else.  So this was reached from a kernel-side stub, and the
   * caller to clone is the user task that trapped, not the code that called
   * into fork.S.
   *
  ...
  if (parent->xcp.sregs != NULL)
    {
      return x86_64_fork_syscall(parent, type);
    }
#endif

  return x86_64_fork_direct(parent, context, type);
}
