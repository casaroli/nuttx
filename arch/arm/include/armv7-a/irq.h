...
struct xcptcontext
{
   ...
   * primitives build the child's context from:  a fork(), vfork() or
   * task_fork() reached through a system call has to give the child the
   * registers of the task that trapped, not those of the kernel-side stub
   * that arm_fork() is called from.  See arm_fork().
   *
