      ...
      default:
        {
           ...
           * primitives need them:  the child of a fork(), vfork() or
           * task_fork() made from user space resumes from this very SVC, so
           * it is built from this frame and not from the registers the
           * kernel-side stub happens to be running with.  Only the outermost
           * system call is of interest, since that is the one the caller
           * made.  See arm_fork().
           */
