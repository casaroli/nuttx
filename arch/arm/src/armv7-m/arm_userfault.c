/****************************************************************************
 * arch/arm/src/armv7-m/arm_userfault.c
 *
 ...
 *   riscv_fault_handler(), and armv7-a and armv8-m in their own
 *   arm_userfault_recover().
 *
 * Input Parameters:
...
bool arm_userfault_recover(uint32_t *regs)
{
   ...
   * redirects a system call to arm_dispatch_syscall(), and
   * exception_common() writes this word back to CONTROL just before the
   * exception return.
   *
  ...
  regs[REG_XPSR] &= ~(ARMV7M_XPSR_ICIIT1_MASK | ARMV7M_XPSR_ICIIT2_MASK);
  regs[REG_XPSR] |= ARMV7M_XPSR_T;
  ...
  putreg32(NVIC_HFAULTS_VECTTBL | NVIC_HFAULTS_FORCED |
           NVIC_HFAULTS_DEBUGEVT, NVIC_HFAULTS);
   ...
   * there, and in a protected build arm_svcall() leaves the stack alone
   * too, so the kernel already runs this task's system calls -- an ordinary
   * exit() included -- on it.  There is nothing to switch to; Cortex-M
