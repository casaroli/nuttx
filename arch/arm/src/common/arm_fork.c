 ...
 *   an ordinary function call, so that the register snapshot the entry point
 *   in fork.S took describes the caller itself.  That is the case in a flat
 *   build, in a protected build -- where a system call is dispatched on the
 *   caller's own stack, so the caller's frames are copied along with the
 *   kernel-side ones and the child unwinds back through them -- and for a
 *   kernel thread in any build.
 *
 * Input Parameters:
...
static pid_t arm_fork_direct(struct tcb_s *parent,
                             const struct fork_s *context, int type)
{
  ...
  child = nxtask_setup_fork((start_t)(context->lr & ~1), type, oldsp);
  if (!child)
    {
 ...
 *   through a system call that was switched onto a kernel stack.  The
 *   register snapshot fork.S took is useless here:  it describes the
 *   kernel-side stub, and the frames below it are on a stack the child does
 *   not get a copy of, so a child built from it would resume at a kernel
 *   address with a stack pointer into its own user stack.
 *
...
static pid_t arm_fork_syscall(struct tcb_s *parent, int type)
{
  ...
  child = nxtask_setup_fork((start_t)(sysreturn & ~1), type, oldsp);
  if (!child)
    {
      serr("ERROR: nxtask_setup_fork failed\n");
      return (pid_t)ERROR;
 ...
 *   child shares the parent's stack -- a borrowing vfork() child -- there is
 *   nothing to relocate and the pointers are carried over unchanged.
 *
 ...
 *      - Allocating the stack, or borrowing the parent's
  ...
  if (parent->xcp.ustkptr != NULL)
    {
      ...
      return arm_fork_syscall(parent, type);
    }
#endif

  return arm_fork_direct(parent, context, type);
}
