 ...
 *   an ordinary function call, so that the register snapshot taken by
 *   arm64_fork_func.S describes the caller itself.  That is the case in a
 *   flat build, and for a kernel thread in any build.
 *
...
static pid_t arm64_fork_direct(struct tcb_s *parent,
 ...
 *   through a system call.  The register snapshot taken by
 *   arm64_fork_func.S is useless here:  it describes the kernel-side stub,
 *   so a child built from it would resume at a kernel address on a kernel
 *   stack.  What the caller was actually doing is the exception frame the
 *   SVC handler recorded in xcp.sregs; the child is built from that.
 *
...
static pid_t arm64_fork_syscall(struct tcb_s *parent, int type)
{
 ...
 *      point in arm64_fork_func.S collects context information and transfers
 *      control to arm64_fork().
 *   2) arm64_fork() calls nxtask_setup_fork().
...
pid_t arm64_fork(const struct fork_s *context, int type)
{
  ...
  if ((parent->flags & TCB_FLAG_SYSCALL) != 0)
    {
      return arm64_fork_syscall(parent, type);
    }
#endif
