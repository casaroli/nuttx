...
struct xcptcontext
{
   ...
   *
   * task_fork(), vfork() and fork() need it:  they are reached through a
   * system call, so the return address and stack pointer their architecture
   * entry point can see for itself are the kernel's, and a child built from
   * those would resume at a kernel address on a kernel stack.
   */
